...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    dtanh = (1 - np.tanh(forward)**2)*(dnext_h)
    dx = dtanh.dot(Wx.T)
    dprev_h = dtanh.dot(Wh.T)
    dWx = x.T.dot(dtanh)
    dWh = dtanh.T.dot(prev_h).T
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    split = np.split(activation, 4, axis=1)
    ai = split[0]
    af = split[1]
    ao = split[2]
    ag = split[3]
    ...
    cache = (x,prev_h,prev_c,Wx,Wh,b,next_h,next_c,ai,af,ao,ag,i,f,o,g)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    x,prev_h,prev_c,Wx,Wh,b,next_h,next_c,ai,af,ao,ag,i,f,o,g = cache
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    prev_c = np.zeros((N,H))
    for t in range(T):
        prev_h, prev_c, local_cache = lstm_step_forward(x[:,t,:],prev_h,prev_c,Wx,Wh,b)
        h[:,t,:] = prev_h
        cache[t] = local_cache
    ##############################################################################
    #                               END OF YOUR CODE                             #
